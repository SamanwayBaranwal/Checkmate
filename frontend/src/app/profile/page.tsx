    ...
    <div className="max-w-3xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
        {[
          ...
          { label: 'Win Rate', value: `${winRate}%`, color: 'text-[#46a883]' },
          { label: 'Win Streak', value: profile.currentStreak || 0, color: profile.currentStreak >= 3 ? 'text-orange-400' : 'text-white' },
          { label: 'Total Earned', value: `$${(profile.totalEarnings || 0).toFixed(2)}`, color: 'text-[#46a883]' },
        ].map((s) => (
              ...
              return (
                <div key={tc.timeControl}>
                  <div className="flex items-center justify-between text-sm mb-1">
                    ...
                    <span className="text-white/50 text-xs">{tc.won}W / {tc.played - tc.won}L — <span className="text-[#46a883] font-semibold">{wr}%</span></span>
                  </div>
      ...
      <div className="card">
        ...
        <div className="flex gap-3">
          <input
            ...
            className="flex-1 bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-[#46a883]"
          />
          <button onClick={saveUsername} disabled={saving || !username} className="btn-primary">
            {saving ? 'Saving...' : 'Save'}
          </button>
        </div>
        {msg && <p className={`text-sm mt-2 ${msg.includes('saved') ? 'text-[#46a883]' : 'text-red-400'}`}>{msg}</p>}
      </div>
            ...
            return (
              <div key={n.key} className="flex items-center justify-between">
                ...
                <button
                  ...
                  className={`relative w-10 h-5 rounded-full transition-colors ${enabled ? 'bg-[#46a883]' : 'bg-white/20'}`}
                >
                  <span
                ...
                return (
                  <tr key={g.id} className="border-b border-white/5 hover:bg-white/5">
                    <td className="px-4 py-2">
                      <span className={`font-semibold ${isDraw ? 'text-yellow-400' : won ? 'text-[#46a883]' : 'text-red-400'}`}>
                    ...
                    <td className={`px-4 py-2 text-right ${won ? 'text-[#46a883]' : isDraw ? 'text-white/50' : 'text-red-400'}`}>
