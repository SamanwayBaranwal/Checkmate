    ...
    <div className="max-w-2xl mx-auto px-4 py-8 space-y-6">
      ...
      <div className="card">
        ...
        <input
          ...
          className="w-full bg-black/30 border border-white/10 rounded-lg px-3 py-2 text-white focus:outline-none focus:border-[#46a883] mb-3"
        />
            ...
            {searchResults.map((u) => {
              ...
              return (
                <div key={u.id} className="flex items-center justify-between bg-white/5 rounded-lg px-3 py-2">
                  ...
                  {isFriend ? (
                    <span className="text-xs text-[#46a883]">Friends</span>
            ...
            {incoming.map((u) => (
              <div key={u.id} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div className={`w-2 h-2 rounded-full ${u.isOnline ? 'bg-[#46a883]' : 'bg-white/20'}`} />
              ...
              {friends.map((f) => (
                <div key={f.id} className="flex items-center justify-between px-4 py-3 hover:bg-white/5">
                  <div className="flex items-center gap-3">
                    <div className={`w-2.5 h-2.5 rounded-full shrink-0 ${f.isOnline ? 'bg-[#46a883] shadow-[0_0_6px_#46a883]' : 'bg-white/20'}`} />
                    <div>
        ...
        <div className="card p-0 overflow-hidden">
          ...
          {leaderboard.length === 0 ? (
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                {leaderboard.map((p, i) => (
                  <tr key={p.id} className={`border-b border-white/5 ${p.isMe ? 'bg-[#46a883]/10' : 'hover:bg-white/5'}`}>
                    <td className="px-4 py-2 text-lg">
                      {i < 3 ? MEDAL[i] : <span className="text-white/40 text-sm">#{i + 1}</span>}
                    </td>
                    <td className="px-4 py-2">
                      <div className="flex items-center gap-2">
                        <div className={`w-2 h-2 rounded-full ${p.isOnline ? 'bg-[#46a883]' : 'bg-white/20'}`} />
                        <Link href={`/profile/${p.id}`} className="font-semibold hover:text-[#f0b232]">
                          {p.username || shortAddr(p.wallet)}
                        </Link>
                        {p.isMe && <span className="text-xs text-[#46a883]">(you)</span>}
                      </div>
                    </td>
                    <td className="px-4 py-2 text-right font-bold text-[#f0b232]">{p.elo}</td>
                    <td className="px-4 py-2 text-right text-[#46a883]">{winRate(p.games_won, p.games_played)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
